-- Binary Semaphore — initial schema (local + CLI migrations).
--
-- This mirrors supabase/schema.sql so `supabase db reset` rebuilds the local
-- database from scratch. The file is idempotent (every statement is
-- create-if-not-exists / drop-then-create), so it stays safe to re-run and to
-- keep in sync with schema.sql. When you change the schema, update BOTH files
-- (or add a new numbered migration here and append to schema.sql).
