            ...
            {projects.map((pr, i) => (
              <div key={i}>
                <div className="flex items-baseline justify-between gap-4">
                  ...
                  {pr.link ? (
                    <span className="shrink-0 text-xs font-normal text-[#0f766e]">
                      {projectLink(pr.link)}
                    </span>
                  ) : null}
                </div>
...
function Section({
  ...
  return (
    <BaseSection className="mt-5" headingClassName="mb-2 text-xs font-bold uppercase tracking-[0.18em] text-[#0f766e]" title={title}>
      {children}
    </BaseSection>
  );
}
