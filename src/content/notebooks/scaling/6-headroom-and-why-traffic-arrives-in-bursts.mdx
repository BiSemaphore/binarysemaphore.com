...
immediately: <Peach>you cannot run a system at 100% utilization and also expect it to
work</Peach>. You need slack.
...
- <Mint>Bounded queues.</Mint> Every queue gets a maximum depth. When it is full, reject
  immediately with 503 and `Retry-After`. <Peach>A fast rejection is a better answer
  than a slow success no one is waiting for.</Peach> This is <Pink>load shedding</Pink>.
