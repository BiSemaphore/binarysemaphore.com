...
clever indirect way: <Peach>an expensive request holds its connection open longer, so
the instance handling it automatically looks busier.</Peach> You do not have to
...
<Peach>Anything that routes `/api` to one service and `/images` to another is
operating at L7, because it had to read the URL to decide.</Peach>
