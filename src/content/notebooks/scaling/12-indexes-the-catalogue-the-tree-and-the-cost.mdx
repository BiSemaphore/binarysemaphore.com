...
The second one is what bites. <Peach>Indexes make reads faster and writes slower,
always.</Peach> A table with twelve indexes pays twelve tree updates on every insert.
...
- <Mint>Index everything else from evidence.</Mint> Wait for the trace that shows the
  ...
  index that column. <Ref to="#14-explain-analyze-deciding-instead-of-guessing">Section 14</Ref>.
...
<Ref to="/postgres">Notebook 04</Ref>, Postgres, sections 25 to 31 go considerably deeper on index types,
