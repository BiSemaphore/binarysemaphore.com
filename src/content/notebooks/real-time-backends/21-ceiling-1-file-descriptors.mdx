...
Not 1,024. <Peach>Seventeen fewer than the round number, and the difference is
enumerable</Peach>:
...
<Peach>The server is up, answering `/healthz`, reporting no errors to your uptime
monitor, and refusing every new connection.</Peach> A liveness probe that checks
...
Every connection is a file descriptor, so <Peach>your connection ceiling is your
descriptor limit minus about seven</Peach>. At the ceiling the server stays healthy and
