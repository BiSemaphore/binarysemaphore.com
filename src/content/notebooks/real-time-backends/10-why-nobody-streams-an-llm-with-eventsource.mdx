...
on an `EventSource`, and it cannot put one on a WebSocket either. <Ref to="#37-authenticating-a-socket">Section 37</Ref> is
the full treatment; the short version is cookie, query-string ticket, or a token
in the first message.

The trade is explicit. Hand-parsing costs you the free reconnection from <Ref to="#9-reconnection-you-did-not-write">section 9</Ref>, because that lived in `EventSource`, not in the format. <Rose>Choosing `fetch` for
