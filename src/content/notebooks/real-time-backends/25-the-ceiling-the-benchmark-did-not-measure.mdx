...
default in most code is the wrong one: <Peach>queue it in application memory and carry
on</Peach>.
...
| **Close the connection** | Correctness needs every message. Let the client reconnect and use the catch-up path of <Ref to="#32-the-catch-up-protocol">section 32</Ref> |

Closing looks brutal and is usually right. <Peach>A client that cannot keep up is
already not seeing the truth</Peach>, and the resume protocol is the mechanism you
