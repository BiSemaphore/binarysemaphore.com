...
stream never connects, and worse, <Peach>ordinary API requests from every tab now
queue behind the streams</Peach>, because they are competing for the same six slots.
...
per-origin connection cap stops applying. <Mint>If you serve SSE over HTTP/2, and any
modern TLS deployment does, this problem is mostly gone.</Mint>
