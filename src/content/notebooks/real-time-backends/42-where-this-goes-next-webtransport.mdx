...
<Mint>So the answer today is still SSE or WebSocket for anything that must work
everywhere</Mint>, with WebTransport as the thing to watch, and the thing to reach for
...
is the right model for state that is superseded immediately. <Peach>Support is not yet
universal</Peach>, so it is the informed answer to "what is next", not the default
choice today.
