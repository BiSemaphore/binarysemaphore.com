...
| Condition | Stops |
|---|---|
| `content-length-range` | The 5 GB upload from <Ref to="#27-the-hole-in-a-presigned-put">section 27</Ref>. <Peach>Enforced by the store</Peach> |
...
using ranged requests so it can resume. <Ref to="/large-scale-ingestion">Notebook 01</Ref> <Ref to="#6-failure-5-durability-is-not-availability">section 6</Ref> covers the
