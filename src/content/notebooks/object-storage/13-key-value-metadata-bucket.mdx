...
is the object's identity, which is why <Ref to="#15-designing-the-key">section 15</Ref> is about designing it.
...
`If-None-Match` for caching (<Ref to="#42-range-requests">section 42</Ref>) and for the conditional writes in
<Ref to="#22-conditional-writes-the-primitive-that-changed-things">section 22</Ref>. If you need a real content hash, ask for one explicitly with the
checksum headers.
